<template>
	<div class="sidebar-menu" style="background: #a53c94;">
		...
		<div class="sidebar-menu-inner">
			...
			<header class="logo-env">
				...
				<div class="sidebar-collapse">
					<a href="#" class="sidebar-collapse-icon" style="color: #fff;"><!-- add class "with-animation" if you want sidebar to have animation during expanding/collapsing transition -->
			...
			<ul id="main-menu" class="main-menu">
				...
				<li v-for="(item, index) in menu" :class="{'has-sub':item.sub_menus.length>0}" :key="index">
					<a v-if="item.sub_menus.length>0" style="color: #fff;">
						<i :class="item.icon"></i>
						<span class="title">{{item.name}}</span>
					</a>
					<router-link :to="item.route" v-else style="color: #fff;">
						<i :class="item.icon"></i>
						<span class="title">{{item.name}}</span>
					</router-link>
					<ul v-if="item.sub_menus.length>0">
						<li v-for="(sub, index) in item.sub_menus" :key="index">
